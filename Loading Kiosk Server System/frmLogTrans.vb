...
#Region "--(02)-- DATA FETCHING AND DISPLAY Functions"
    ...
    Private Function getLog() As Boolean
        ...
        '=================================================================|
        Dim filterQuery As String = "WHERE (log_trans.logDateTime >= '" & convertToDateDB(globalDtFrom) & " 00:00:00' " & _
                                   "AND log_trans.logDateTime <= '" & convertToDateDB(globalDtTo) & " 23:59:59') "
        If sortByMach <> "" Then
        ...
        sql = New MySqlCommand("SELECT " & _
                             "log_trans.logNo AS 'LOG NO.', " & _
                             "DATE_FORMAT(log_trans.logDateTime,'%Y-%m-%d %h:%i %p') AS 'DATETIME', " & _
                             "log_trans.refNo AS 'REF. NO.', " & _
                             "man_machine.machName AS 'MACHINE', " & _
                             "log_trans.prodNo AS 'PRODUCT', " & _
                             "log_trans.mobileNo AS 'MOBILE NUMBER', " & _
                             "log_trans.totalDue AS 'TOTAL DUE', " & _
                             "log_trans.totalInsert AS 'INSERTED BILL / COIN' " & _
                             "FROM log_trans " & _
                             "LEFT JOIN man_machine ON man_machine.machNo = log_trans.machNo " & _
                             "" & filterQuery & " " & _
                             "", con)
    ...
    Private Sub getLatestDates()
        ...
        Try
            Dim dateFrom As String = getDbValue("SELECT logDateTime FROM log_trans ORDER BY logDateTime DESC LIMIT 0,1", False)
            If dateFrom <> "" Then
...
#Region "--(03D)-- SuperGridControl1DataBindingComplete"
    ...
    Private Sub CustomizePanel1(ByVal panel As GridPanel)
        ...
        With panel
            ...
            .Columns(3).CellStyles.Default.Alignment = Alignment.MiddleLeft
            .Columns(4).CellStyles.Default.Alignment = Alignment.MiddleLeft
            .Columns(5).CellStyles.Default.Alignment = Alignment.MiddleLeft
            .Columns(6).CellStyles.Default.Alignment = Alignment.MiddleRight
            .Columns(7).CellStyles.Default.Alignment = Alignment.MiddleRight
